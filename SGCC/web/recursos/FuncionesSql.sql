...
DELIMITER $$
CREATE FUNCTION NUEVO_SOPORTE_INGRESO(IDINGRESO INTEGER, IDSOPORTE VARCHAR(200), SOPORTE BLOB)
RETURNS VARCHAR(200)
BEGIN	
    INSERT INTO SOPORTE_INGRESO VALUES(DEFAULT,IDSOPORTE,SOPORTE, IDINGRESO);
...
DELIMITER $$
CREATE PROCEDURE LEER_SOPORTES_DE_INGRESO(IDINGRESO INTEGER)
BEGIN
...
DROP PROCEDURE IF EXISTS NOMBRE_CONCEPTO_INGRESO;
DELIMITER $$
CREATE PROCEDURE NOMBRE_CONCEPTO(IDCONCEPTO INTEGER)
BEGIN
	SELECT NOMBRE FROM CONCEPTO_INGRESO WHERE IDENTIFICADOR=IDCONCEPTO;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS NOMBRE_CONCEPTO_GASTO;
DELIMITER $$
CREATE PROCEDURE NOMBRE_CONCEPTO_GASTO(IDCONCEPTO INTEGER)
BEGIN
	SELECT NOMBRE FROM CONCEPTO_GASTO WHERE IDENTIFICADOR=IDCONCEPTO;
END$$
DELIMITER ;
